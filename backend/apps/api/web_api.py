...
# When both are set, all routes (API + frontend) require HTTP Basic Auth.
# The /api/health endpoint is always exempt so Docker healthchecks keep working.
...
def _apply_basic_auth():
    """Enforce HTTP Basic Auth when BASIC_AUTH_USER and BASIC_AUTH_PASS are set."""
    if not _BASIC_AUTH_ENABLED:
        return None

    # Always allow health check (Docker healthcheck / uptime monitors)
    ...
    if auth_header.startswith('Basic '):
        try:
            decoded = _base64.b64decode(auth_header[6:]).decode('utf-8', errors='replace')
    ...
    return Response(
        ...
        401,
        {'WWW-Authenticate': 'Basic realm="StreamFlow"'}
    )
...
def rescore_resort():
    """Re-score and re-sort all channels using current scoring config (no FFmpeg)."""
    try:
        service = get_stream_checker_service()
        result = service.rescore_and_resort()
        if result.get('success'):
            return jsonify(result), 202
        return jsonify(result), 500
    except Exception as e:
        logger.error(f"Error triggering rescore & resort: {e}")
        return jsonify({"error": "Internal Server Error"}), 500


@app.route('/api/stream-checker/test-streams-without-stats', methods=['POST'])
def test_streams_without_stats():
    """Find and queue streams with missing or incomplete quality stats."""
    try:
        service = get_stream_checker_service()
        result = service.test_streams_without_stats()
        if result.get('success'):
            return jsonify(result), 202
        return jsonify(result), 500
    except Exception as e:
        logger.error(f"Error triggering test without stats: {e}")
        return jsonify({"error": "Internal Server Error"}), 500
...
def trigger_global_action():
    ...
    try:
        service = get_stream_checker_service()
        result = service.trigger_global_action()
        if result.get('success'):
            return jsonify(result), 202
        return jsonify(result), 500
    except Exception as e:
