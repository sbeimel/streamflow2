...
def get_stream_proxy(stream_id: int) -> Optional[str]:
    """Get HTTP proxy URL for a stream from its M3U account configuration.

    Retrieves the proxy setting from the M3U account associated with the stream.
    The proxy is passed to FFmpeg via -http_proxy so quality checks use the same
    network path as regular channel streaming.

    Args:
        stream_id: The stream ID to look up

    Returns:
        Proxy URL string (e.g. 'http://proxy:8080') or None if not configured
    """
    try:
        udi = get_udi_manager()
        ...
        m3u_account_id = stream.get('m3u_account') or stream.get('m3u_account_id')
        if not m3u_account_id:
            return None

        account = udi.get_m3u_account_by_id(m3u_account_id)
        if not account:
            return None

        proxy = account.get('proxy')
        if proxy and proxy.strip():
            logger.debug(f"Found proxy '{proxy.strip()}' for stream {stream_id} (account {m3u_account_id})")
            return proxy.strip()

        return None
    except Exception as e:
        logger.debug(f"Could not fetch proxy for stream {stream_id}: {e}")
        return None
