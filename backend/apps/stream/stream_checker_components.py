...
class StreamCheckConfig:
    ...
    DEFAULT_CONFIG = {
        ...
        'scoring': {
            'method': 'legacy',  # 'legacy' (linear weighted) or 'enhanced' (MACstrom sigmoid)
            'weights': {
                'bitrate': 0.40,
                'resolution': 0.35,
                'fps': 0.15,
            ...
            'prefer_h265': True,  # prefer h265 over h264 (legacy only)
            'avoid_h265': False   # penalize h265/hevc (both methods, mutual exclusive with prefer_h265)
        },
        'stream_ordering': {
            'provider_diversification': False,  # interleave streams from different providers
            'diversification_mode': 'round_robin'  # 'round_robin' or 'priority_weighted'
        },
        'queue': {
            'max_size': 1000,
        ...
        'quality_check_exclusions': {
            'enabled': False,  # Enable quality check exclusions for specific M3U accounts
            'excluded_accounts': []  # List of M3U account IDs to exclude from FFmpeg analysis (use priority-based scoring instead)
        },
        'stream_check_immunity': {
            'enabled': True,   # Enable stream check immunity (skip recently checked streams)
            'duration_hours': 2  # Hours a stream is immune from re-checking (0 = always recheck, max 720)
        },
        'account_stream_limits': {
            'enabled': False,  # Enable per-account stream limits per channel
            'global_limit': 0,  # Max streams per M3U account per channel (0 = unlimited)
            'account_limits': {}  # Per-account overrides: {"account_id": limit} (0 = unlimited)
        },
        'profile_failover': {
            'enabled': False,  # Retry failed stream checks with alternative M3U account profiles
        },
        'resolution_preference': {
            'mode': 'default',  # 'default' | 'prefer_4k' | 'avoid_4k' | 'max_1080p' | 'max_720p'
        }
    }
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        Uses deep merge so nested keys added in new versions are preserved.
        ...
        def _deep_update(base: Dict, updates: Dict) -> None:
            """Recursively merge updates into base, preserving nested defaults."""
